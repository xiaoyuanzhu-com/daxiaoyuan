...
本 spec 定义这个后端的**第一版（scope A）**——只读接口 + 静态城市配置 + 学校表。
...
- **用户共建**（notes / confirms / 提交反馈）—— scope B
- **审核 / 后台 / 写接口** —— scope B
- **账号体系 / openid 收集** —— scope B
...
| 语言 | Go 1.22+ | PRD 指定 |
| HTTP 框架 | Gin | PRD 指定 |
| 数据库 | SQLite（单文件） | PRD 指定。MVP 量级 < 1k 行学校 + 共建表，sqlite 足够 |
