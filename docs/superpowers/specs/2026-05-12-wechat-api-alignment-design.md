...
dev 时需要打到本地后端（`http://localhost:8080`）。小程序没有标准的 build-time env 机制（不像 Vite 的 `.env.*`）。**采用人工切换 + git 警示**：

```js
// wechat/app.js
App({
  globalData: {
    // ⚠️ DEV: 'http://localhost:8080' / PROD: 'https://ddxy.xiaoyuanzhu.com'
    // 发布前必须改回 production!
...
`README.md`（或 wechat 目录下的 README，若有）记一句"发布前 checklist：apiBase 设为 production"。

**为什么不更精巧**：小程序没有 build-time 注入；用 `__wxConfig.envVersion` 区分 dev/release 是动态判断,可行但增加复杂度。MVP 阶段一行注释 + 一行配置足够。
...
- **dev URL** 是用 `http://localhost:8080` 还是局域网 IP？小程序真机调试时 localhost 不通,要用 dev 机器的 IP。倾向：dev 用 `localhost`（开发者工具模拟器内有效）,真机调试时手动改成 IP,plan 文档里记一句即可。
- **`vendor.png` / `tabbar` 图标**：和本次改动无关,不动。
