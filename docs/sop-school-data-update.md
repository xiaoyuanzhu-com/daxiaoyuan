...
> 一所学校一次"data update pass"的标准动作。覆盖：核对当前开放状态、填齐字段、提交 PUT、记录待办。

## 何时跑这套 SOP

- 一所学校第一次进系统时（首次入库）
- 距上次 `lastUpdate` 超过 3 个月、且没有用户共建数据可信地推翻它时
- 收到学校开放政策变更的官方公告 / 媒体报道，需要核对实情时

## Step 0 — 准备

- 本地后端跑起来：`cd backend && make run`
- 知道目标学校的 slug（域名规则：`https://www.<X>.edu.cn` 的 `<X>`）
- 准备一个空文档放调研笔记（备查、备追溯）
...
明确"什么字段已有、什么字段是占位（`placehold.co` 之类）、什么字段为空"。
...
按下表逐项填。**"无相反证据 → 保留现值"** 是默认策略——别因为"看上去不对"就改。

| 字段 | 决策依据 | 备注 |
|---|---|---|
| `address` | 校官网联系方式页 / 保卫部页面 | 多校区学校：选**与 lat/lng 一致**的那个校区，并在后缀加「（东校区）/（主校区）」消歧 |
...
| `facilities.*.reservation` | 仅 `status=appt` 时填 | 写**真实操作路径**到 hint，例："关注「中国农大图书馆」公众号 → 菜单「入馆预约」" |
...
3. `logo` 字段直接写 prospective CDN URL：`https://static.ddxy.xiaoyuanzhu.com/images/logo/<id>.svg`——上传发生前会 404，但前端 `<img onError>` 会优雅隐藏（见 `frontend/src/screens/EditScreen.jsx`），影响可接受。
...
服务器会自动写 `lastUpdate`，不要在 body 里传它。
...
返回 200 + 完整 school 对象 = 成功。`facilities` 必须包含全部 4 个 key（library / track / gym / canteen），缺一个会 400。

## Step 5 — 记 TODO 清单

每次 pass 完，把下面这些挂到一个统一的"补丁"清单（建议放 `docs/data-todos.md`）：
...
本地写入完成后，prod 仍是旧数据。同步到 `ddxy.xiaoyuanzhu.com` 的工作流目前**未文档化**——按 docker compose 部署直接覆盖 sqlite 文件，或在 prod 上重复 PUT。等同步流程定型后补到本 SOP。

## 参考案例：中国农业大学（cau）

详见 [docs/sop-walkthrough-cau.md](sop-walkthrough-cau.md)（首次 walk-through 的完整调研笔记 + 决策记录）。
